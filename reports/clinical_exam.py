...
def clinical_exam_initial (file_number, user_name):
    ...
    while not check:
        ...
        lump_location = ask.ask_option("Location of lump", ["Right Breast", "Left Breast",
                                                                          "Both", "Not present"])
        lump_location_data = []
        ...
        if lump_location == "Not present":
            ...
            lump_size = ask.ask_option("Lump size", ["< 2cm", "2-5 cm", ">5 cm"])
            lump_number = ask.ask_option("Number of lumps", ["Single", "Multiple", "Other"])
            lump_consistency = ask.ask_option("Consistency of lumps", ["Soft", "Firm", "Hard", "Cystic",
                                                                                     "Mobile", "Other"])
            lump_fixity = ask.ask_option("Lump fixity to ", ["Skin", "Chest wall", "Pectoral major muscle",
                                                                           "No Fixation", "Other"])
        mastitis_location = ask.ask_option("Location of mastitis",
        ...
        if mastitis_location == "Not present":
            ...
            mastitis_type = ask.ask_option("Mastitis type", ["Diffuse", "Sectoral", "Other"])
        tender = ask.ask_option("Tenderness in breast ?",["Right Breast", "Left Breast", "Both",
                                                                        "Not Present", "Other"])
        retract = ask.ask_option("Nipple Retraction ?",["Right Breast", "Left Breast", "Both",
                                                                      "Not Present", "Other"])
        discharge = ask.ask_option("Nipple Discharge ?",
                                     ["Right Breast", "Left Breast", "Both", "Not Present", "Other"])
        if discharge == "Not Present":
            discharge_type = "NA"
        else:
            discharge_type = ask.ask_option("Discharge Type?",
                                     ["Serous", "Milky", "Brown", "Bloody", "Other"])
        skin_change_location = ask.ask_option("Skin Changes?",
                                     ["Right Breast", "Left Breast", "Both", "Not Present", "Other"])
        ...
        ax_nodes = ask.ask_option("Palpable axillary nodes",
        ...
        if ax_nodes == "Not palpable":
            ...
            ax_nodes_fixity =  ask.ask_y_n("Fixity of axillary nodes", "Yes", "No")
        supra_nodes = ask.ask_option("Palpable supraclavicular nodes",
                                                ["Right Breast", "Left Breast", "Both", "Not palpable", "Other"])
        if supra_nodes == "Not palpable":
            ...
            supra_nodes_fixity = ask.ask_y_n("Fixity of supraclavicular nodes", "Yes", "No")
        contra_breast = ask.ask_option("Contralateral Breast", ["Normal", "Diffuse Mastitis",
                                                                              "Localised Mastitis", "Other"])
        arm_edema = ask.ask_option("Edema of arm", ["Right", "Left", "Both", "Not Present", "Other"])
        ...
        data_list = [prov_diag, lump_palp, lump_location_data, lump_size, lump_number, lump_consistency,
...
def nipple_cytology (file_number):
    ...
    while not check:
        ...
        if cyto == "Not Done":
            ...
            cyto_report = ask.ask_option("Nipple Cytology report and interpretation", ["Normal",
                                                                                                     "Suspicious",
                                                                                                     "Diagnostic for "
                                                                                                     "Cancer", "Other"])
...
def add_data(conn, cursor, file_number, user_name):
    ...
    if enter:
        ...
        add_update_sql.update_multiple(conn, cursor, table, pccm_names.name_clinical("nipple_cytology"), file_number,
                                       data)
    # enter = ask.ask_y_n("Enter results of other clinical tests?")
    # if enter:
